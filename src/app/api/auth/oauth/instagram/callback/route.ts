...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      return NextResponse.redirect(
        new URL('/dashboard?oauth_error=get_account_failed', req.nextUrl.origin)
      )
    }
    ...
    if (!instagramUserId || !instagramUsername) {
      ...
      return NextResponse.redirect(
        new URL('/dashboard?oauth_error=no_account_found', req.nextUrl.origin)
      )
    }
    ...
    const response = NextResponse.redirect(
      new URL('/dashboard?oauth_success=instagram', req.nextUrl.origin)
    )
